...
use crate::rpc::{fetch_balance, send_corrupted_metadata_tx, import_wallet_key, Connection};
use crate::metadata::CorruptedMetadata;
...
pub fn run(connection: &Connection) -> Result<()> {
    ...
    // Prepare dummy metadata with wrong formatting
    let cid = Cid::try_from("baga6ea4seaqcbzdyshqeqxw2hw2nbv2a45vruq54mc7f3ukgdtqjmdv7n7p7gqq")?;
    let metadata = CorruptedMetadata {
        provider: 1000,
        data: cid,
        size: PaddedPieceSize(1 << 10),
        term_min: 100,
        term_max: 200,
        //expiration: 500, Corrupted metadata do not have any expiration
    };
